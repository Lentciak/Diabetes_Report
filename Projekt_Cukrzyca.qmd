...
```{r Funkcja wykresu macierzy konfuzji}
...
ggplot(matrix, aes(Target, Prediction, fill = Freq)) + 
  geom_tile() +
...
Na podstawie badania przeprowadzonego w 2015 roku powstał zbiór danych [@Teboul_2021], który jest bazą dla niniejszego projektu. Składa się z 229474 obserwacji opisanych przez 20 zmiennych, które zostały opisane poniżej.
...
Zmienna BMI ma `r outliers_BMI` wartości odstających, co przekłada się na `r round((outliers_BMI/nrow(dane.train))*100, 2)`% jej wartości w zbiorze testowym. Jest to bardzo niewielka ilość, dlatego pominiemy ich istotność przy treningu modelu. Kolejnym powodem tej decyzji jest konsensus badaczy pod względem nieusuwania obserwacji odstających ze zbiorów danych o charakterze medycznym tak długo, jak zawarte w nich dane mają rzeczywisty sens, a nie są błędnymi wartościami.
...
Zmienne mówiące o wieku, nadciśnieniu i BMI respondentów zgodnie z wynikami testów statystycznych okazały się być istotnymi predyktorami zachorowania na cukrzycę. To podejrzenie nie potwierdziło się jedynie dla zmiennej opisującej zarobki badanych.

```{r Tabela 5 najważniejszych predyktorów dla każdego modelu, echo=F}
#| label: tbl-varimp
#| tbl-cap: Tabela istotności predyktorów dla poszczególnych modeli klasyfikacyjnych
