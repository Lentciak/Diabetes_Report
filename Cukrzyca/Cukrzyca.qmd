...
zmienne_jakosciowe %>%
  ...
  labs(x = 'Czy respondent jest diabetykiem', y = 'Liczba respondentów') +
  theme_minimal() +
  theme(legend.title = element_blank(),
...
```{r Tabela współczynnika kontyngencji V-Cramera dla poszczególnych par zmiennych kategorycznych}
#| echo: false
...
```{r}
#| label: tbl-cramer
#| tbl-cap: Tabela współczynnika kontyngencji V-Cramera dla poszczególnych par zmiennych nominalnych
#| output: page
cramerV_table %>%
  arrange(-cv) %>%
  kbl(col.names = c('Zmienna 1', 'Zmienna 2', 'Wartość współczynnika', 'DoF')) %>%
  kable_minimal() %>%
